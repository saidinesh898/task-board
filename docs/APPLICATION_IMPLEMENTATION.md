...
npm run build     Vinext Sites server bundle in dist
npm run build:next
                  Native Next.js production build and TypeScript validation
...
The repository deliberately supports two production adapters:

- the default `npm run build` uses Vinext, which reimplements the Next.js API surface on Vite and emits the Sites-compatible RSC/server bundle under `dist/server/index.js`;
- the `postbuild` script copies `.openai/hosting.json` into the generated artifact so the host can associate the bundle with the correct site project;
- Docker and Nixpacks run `npm run build:next` with `TASK_BOARD_BUILD_TARGET=standalone`. Native Next.js then emits a traced minimal Node server and only required runtime dependencies under `.next/standalone`.

Vinext's compatibility scanner recognizes the App Router, route structure, UI
libraries, and supported configuration. It reports only the known Google Font
behavior difference: Vinext loads it from the CDN instead of self-hosting it at
build time. The Docker target remains the native Next.js path.
...
3. `builder` copies dependencies and source, then runs `npm run build:next` with `TASK_BOARD_BUILD_TARGET=standalone`.
...
`nixpacks.toml` explicitly selects Node 22, runs `npm ci`, builds with `TASK_BOARD_BUILD_TARGET=standalone npm run build:next`, and starts with `npm run start`. It exists because Dokploy may default to Nixpacks if Dockerfile mode is not selected.
...
| Vinext + Vite RSC plugins | Sites-compatible Vite/RSC production adapter and `dist/server/index.js` bundle |
| React/React DOM 19.2.8 | Component and hook runtime |
