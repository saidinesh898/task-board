    ...
    <form onSubmit={submit}>
      ...
      <div className={styles.fieldGrid}>
        <Field label="Assignee" htmlFor={`${prefix}-assignee`} error={errors.assignee}>
          <Select value={value.assignee || null} onValueChange={(assignee) => onChange({ assignee: assignee ?? "" })}>
            <SelectTrigger id={`${prefix}-assignee`} className={styles.fullWidth}><SelectValue placeholder="Choose a person" /></SelectTrigger>
        ...
        <Field label="Priority" htmlFor={`${prefix}-priority`}>
          <Select value={value.priority} onValueChange={(priority) => priority && onChange({ priority })}>
            <SelectTrigger id={`${prefix}-priority`} className={styles.fullWidth}><SelectValue /></SelectTrigger>
            <SelectContent>{PRIORITIES.map((priority) => <SelectItem key={priority} value={priority} className={styles.capitalize}>{priority}</SelectItem>)}</SelectContent>
          </Select>
        </Field>
      </div>
      {showStatus && <Field label="Status" htmlFor={`${prefix}-status`}>
        <Select items={STATUS_LABELS} value={value.status} onValueChange={(status) => status && onChange({ status })}>
          <SelectTrigger id={`${prefix}-status`} className={styles.fullWidth}><SelectValue /></SelectTrigger>
      ...
      <Button type="submit" className={styles.fullWidth}>{submitLabel}</Button>
      </fieldset>
    </form>
  )
}
...
function Field({ label, htmlFor, error, hint, children }: { label: string; htmlFor: string; error?: string; hint?: string; children: React.ReactNode }) {
  return <div className={styles.field}><div className={styles.labelRow}><Label htmlFor={htmlFor}>{label}</Label>{hint && <span className={styles.hint}>{hint}</span>}</div>{children}{error && <p className={styles.error} role="alert">{error}</p>}</div>
}
